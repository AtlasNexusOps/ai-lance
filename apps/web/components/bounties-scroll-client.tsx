...
export function BountiesScrollClient({ items }: { items: ApiBounty[] }) {
  return (
    <section className="mx-auto w-full max-w-5xl px-4 pb-20">
      ...
      <motion.div
        className="-mx-4 flex gap-4 overflow-x-auto px-4 pb-2 scrollbar-none snap-x snap-mandatory"
      ...
      >
        {items.map((bounty, i) => (
          <BountyMiniCard key={bounty.id ?? i} bounty={bounty} index={i} />
        ))}
      </motion.div>
    </section>
  );
...
function BountyMiniCard({
  ...
  return (
    <motion.article
      className="glass shrink-0 w-72 snap-start rounded-2xl p-5 hover:shadow-glass-strong transition-shadow cursor-pointer"
